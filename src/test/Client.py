...
def sign_in():
    ...
    while True:
        try:
            authServer = ssl_client(global_config['AuthServer']['ip'], global_config['AuthServer']['port'])
            print('权限服务器连接成功')
...
def access_application(appserver_ip:str, credential: str):
    ...
    while True:
        try:
            ...
            print('应用服务器连接成功')
            print(f'[{ctime()}] 来自 ' + appserver_ip + ' 的消息: ' + ssl_authServer.recv(1024).decode('utf-8'))
            break
        except Exception as e:
    ...
    while True:
        try:
            # 发送应用验证消息
            ssl_authServer.send(pack_mess(uIP=local_public_ip, uID=user_accout, sIP=appserver_ip, sID=global_config['AppServer']['id'], cre='', mess_type='cre', mess=f'{credential}'))
